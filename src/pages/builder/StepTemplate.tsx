...
export function StepTemplate(props: BuilderContext) {
  ...
  const loadSample = (tmpl: string) => {
    const sample = TEMPLATE_SAMPLES[tmpl];
    if (!sample) return;
    // Warn before overwriting existing work.
    if (builderPois.length > 0) {
      const msg = uiLang === "ja"
    ...
    // Switch mode to match the sample, then load categories + pois.
    const nextCfg: AppConfig = sample.mode === "indoor"
      ...
      <div className="card">
        ...
        {selectedTemplate && TEMPLATE_PREVIEWS[selectedTemplate] ? (
          <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--card2)", borderRadius: 12, border: "1px solid var(--line)" }}>
            ...
            {TEMPLATE_SAMPLES[selectedTemplate] ? (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px dashed var(--line)" }}>
                <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 8 }}>
                  {uiLang === "ja"
                    ? "白紙から始めるのが不安なら、サンプルの地点を入れて完成形から始められます。"
                    : "Not sure where to start? Load sample points and begin from a working example."}
                </div>
                <button type="button" className="btn soft" onClick={() => loadSample(selectedTemplate)}>
                  {uiLang === "ja"
                    ? `サンプルを入れて始める（${TEMPLATE_SAMPLES[selectedTemplate].pois.length}地点）`
                    : `Load sample (${TEMPLATE_SAMPLES[selectedTemplate].pois.length} points)`}
                </button>
                <div className="hint" style={{ marginTop: 6 }}>
                  {uiLang === "ja"
                    ? "入れたあと STEP 2 で自由に編集・削除できます。屋内テンプレートはフロア画像のアップロードもお忘れなく。"
                    : "You can freely edit or delete them in STEP 2. For indoor templates, remember to upload a floor image."}
                </div>
              </div>
            ) : null}
          </div>
