...
import { parseCategoriesFromCsv, parsePoisFromCsv, exampleCategoriesCsv, examplePoisCsv, mergePois, mergeCategories } from "../lib/csv";
...
import { metricsCount } from "../lib/builderMetrics";
...
const onUndo = useCallback(() => {
  ...
  const TEMPLATE_PREVIEWS: Record<string, {
    ...
    tourism: {
      ja: "観光マップ", en: "Tourism", icon: "🏯", mode: "outdoor",
      needs: ["お土産", "駅", "飯屋"],
      categories: [
    ...
    live: {
      ja: "ライブ", en: "Live event", icon: "🎤", mode: "outdoor",
      needs: ["ホテル", "駅", "飯屋"],
      categories: [
    ...
    convenience: {
      ja: "便利マップ", en: "Convenience", icon: "🗺️", mode: "outdoor",
      needs: [],
      categories: [
    ...
    festival: {
      ja: "祭り", en: "Festival", icon: "🎆", mode: "outdoor",
      needs: ["駐車場", "トイレ", "救護"],
      categories: [
    ...
    school_festival: {
      ja: "文化祭", en: "School fest", icon: "🏫", mode: "indoor",
      needs: [],
      categories: [
    ...
    disaster: {
      ja: "防災", en: "Disaster", icon: "🚨", mode: "indoor",
      needs: ["AED", "避難所", "給水所"],
      categories: [
    ...
    outdoor_activity: {
      ja: "アウトドア", en: "Outdoor", icon: "🏕️", mode: "outdoor",
      needs: ["給水所", "コンビニ", "駐車場"],
      categories: [
  ...
  const applyTemplate = (tmpl: string) => {
    ...
    // Switch the indoor/outdoor mode to match the template.
    const prevCfg = builderConfig as AppConfig;
    const nextMode = info.mode ?? prevCfg.mode;
    const modeCfg: AppConfig = nextMode === "indoor"
      ? { ...prevCfg, mode: "indoor", indoor: { ...prevCfg.indoor } }
      : { ...prevCfg, mode: "outdoor" };
    setBuilderConfig({ ...modeCfg, template: tmpl as AppConfig["template"], reco: { needs: info.needs, rules: (builderConfig?.reco?.rules ?? {}) } });
  };
  ...
  // Floor-plan editor: null = closed, "" = single-floor target, else floor id.
  const [floorEditorTarget, setFloorEditorTarget] = useState<string | null>(null);
  ...
  // ── CSV file import with merge mode (v10.23.0) ──
  // "merge"  : append new records; a record whose id/category already exists
  //            OVERWRITES the existing one (whole record). Lets several
  //            people prepare separate CSVs and pour them in one by one.
  // "replace": discard current records of that type and use the file as-is.
  const [csvImportMode, setCsvImportMode] = useState<"merge" | "replace">("merge");

  const importPoisCsvFile = useCallback(async (file: File) => {
    if (!cfg) return;
    metricsCount("csv_import_pois");
    let text = "";
    try { text = await file.text(); } catch {
      toast.error(uiLang === "ja" ? "ファイルを読み込めませんでした" : "Could not read the file.");
      return;
    }
    const incoming = parsePoisFromCsv(text);
    if (incoming.length === 0) {
      toast.error(uiLang === "ja"
        ? "有効な地点が見つかりませんでした（1行目の列名に id,name などがあるか確認してください）"
        : "No valid places found (check the header row: id, name, ...).");
      return;
    }
    const cats = ensureDefaultCategory(uiLang, builderCategories);
    const catKey = cats[0]?.category ?? "general";
    const fixed = incoming.map(p => ({ ...p, category: p.category || catKey }));

    if (csvImportMode === "replace") {
      setBuilderData(fixed, cats);
      setPoisCsv(poisToCsv(fixed, supportedLangs, defaultLang));
      if (!builderCategories.length) setCatsCsv(categoriesToCsv(cats, supportedLangs, defaultLang));
      toast.success(uiLang === "ja"
        ? `地点CSVを置き換えました（${fixed.length}件）`
        : `Places replaced (${fixed.length}).`);
      return;
    }

    const res = mergePois(builderPois, fixed);
    setBuilderData(res.merged, cats);
    setPoisCsv(poisToCsv(res.merged, supportedLangs, defaultLang));
    if (!builderCategories.length) setCatsCsv(categoriesToCsv(cats, supportedLangs, defaultLang));
    const skippedNote = res.skipped > 0
      ? (uiLang === "ja" ? `・ID空欄で${res.skipped}件スキップ` : `, ${res.skipped} skipped (empty id)`)
      : "";
    toast.success(uiLang === "ja"
      ? `マージしました: 追加${res.added}件・上書き${res.updated}件${skippedNote}（合計${res.merged.length}件）`
      : `Merged: ${res.added} added, ${res.updated} overwritten${skippedNote} (${res.merged.length} total).`);
  }, [cfg, csvImportMode, builderPois, builderCategories, setBuilderData, uiLang, supportedLangs, defaultLang]);

  const importCatsCsvFile = useCallback(async (file: File) => {
    if (!cfg) return;
    metricsCount("csv_import_cats");
    let text = "";
    try { text = await file.text(); } catch {
      toast.error(uiLang === "ja" ? "ファイルを読み込めませんでした" : "Could not read the file.");
      return;
    }
    const incoming = parseCategoriesFromCsv(text);
    if (incoming.length === 0) {
      toast.error(uiLang === "ja"
        ? "有効なカテゴリが見つかりませんでした（1行目の列名に category があるか確認してください）"
        : "No valid categories found (check the header row: category, ...).");
      return;
    }
    let next: typeof incoming;
    let msg: string;
    if (csvImportMode === "replace") {
      next = ensureDefaultCategory(uiLang, incoming);
      msg = uiLang === "ja" ? `カテゴリCSVを置き換えました（${next.length}件）` : `Categories replaced (${next.length}).`;
    } else {
      const res = mergeCategories(builderCategories, incoming);
      next = ensureDefaultCategory(uiLang, res.merged);
      msg = uiLang === "ja"
        ? `マージしました: 追加${res.added}件・上書き${res.updated}件（合計${next.length}件）`
        : `Merged: ${res.added} added, ${res.updated} overwritten (${next.length} total).`;
    }
    setBuilderData(builderPois, next);
    setCatsCsv(categoriesToCsv(next, supportedLangs, defaultLang));
    toast.success(msg);
  }, [cfg, csvImportMode, builderPois, builderCategories, setBuilderData, uiLang, supportedLangs, defaultLang]);
...
<button
  type="button"
  className={"savePill savePillBtn " + (csvApplyState === "pending" ? "unsaved" : "saved")}
  onClick={onApplyToCsv}
  disabled={csvApplyState !== "pending"}
  title={uiLang === "ja"
    ? (csvApplyState === "pending" ? "編集内容を保存します" : "保存済みです")
    : (csvApplyState === "pending" ? "Save your changes" : "All changes saved")}
>
  {csvApplyState === "pending"
    ? "💾 " + (uiLang === "ja" ? "保存する" : "Save")
    : "✅ " + (uiLang === "ja" ? "保存済み" : "Saved")}
</button>
          </div>
        </div>
          ...
          <div className="cards">
            ...
            <div className="card">
              ...
              <div className="hint mb12">
                {uiLang === "ja"
                  ? "テンプレートを選ぶと、屋内/屋外モードとおすすめ表示が自動で設定されます。あとから変更できます。"
                  : "Selecting a template sets the indoor/outdoor mode and recommended spots. You can change them later."}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>
                        {info.mode === "indoor" ? (uiLang === "ja" ? "屋内" : "Indoor") : (uiLang === "ja" ? "屋外" : "Outdoor")}
                      </div>
                      {info.needs.length > 0 ? (
              ...
              {selectedTemplate && TEMPLATE_PREVIEWS[selectedTemplate] ? (
                <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--card2)", borderRadius: 12, border: "1px solid var(--line)" }}>
                  <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
                    {uiLang === "ja" ? "表示モード:" : "Map mode:"}
                  </div>
                  <div style={{ marginBottom: 8 }}>
                    <span className="badge">
                      {TEMPLATE_PREVIEWS[selectedTemplate].mode === "indoor" ? (uiLang === "ja" ? "屋内" : "Indoor") : (uiLang === "ja" ? "屋外" : "Outdoor")}
                    </span>
                  </div>
                  <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
...
{cfg.mode === "outdoor" ? (
  <>
    ...
    <div className="hint" style={{ gridColumn: "1 / -1", background: "var(--card2)", borderRadius: 10, padding: "8px 12px" }}>
        ...
        {uiLang === "ja" ? (
          ...
          <>
            ...
            <span className="dispBlock">• Closed day: <code>Wed</code> / <code>Mon,Tue</code> / no closed day → leave blank</span>
                ...
                <div className="hint" style={{ marginTop: 10, textAlign: "right" }}>
                  {uiLang === "ja"
                    ? "編集内容は画面上部の「💾 保存する」ボタンで保存できます。"
                    : 'Use the "💾 Save" button at the top to save your changes.'}
                </div>
                </>
              ) : (
                <div className="mt10">
                  ...
                  {/* CSV file import with merge mode (multi-person workflow) */}
                  <div className="card" style={{ marginTop: 10 }}>
                    <div className="sectionTitleSm">{uiLang === "ja" ? "CSVファイルから取り込む" : "Import from CSV file"}</div>
                    <div className="hint mb10">
                      {uiLang === "ja"
                        ? "分担して作ったCSV（Googleスプレッドシートのダウンロード等）を順番に取り込めます。「マージ」なら既存データに追記し、IDが同じ行は上書きされます。取り込みは「元に戻す」で取り消せます。"
                        : "Pour in CSVs prepared by different people (e.g. downloaded from Google Sheets). \"Merge\" appends to current data and overwrites rows with the same ID. Imports can be undone."}
                    </div>
                    <div className="row" style={{ gap: 14, flexWrap: "wrap", alignItems: "center" }}>
                      <label style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 13, cursor: "pointer" }}>
                        <input type="radio" name="csvImportMode" checked={csvImportMode === "merge"} onChange={() => setCsvImportMode("merge")} />
                        {uiLang === "ja" ? "マージ（追記・ID重複は上書き）" : "Merge (append; same ID overwrites)"}
                      </label>
                      <label style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 13, cursor: "pointer" }}>
                        <input type="radio" name="csvImportMode" checked={csvImportMode === "replace"} onChange={() => setCsvImportMode("replace")} />
                        {uiLang === "ja" ? "置き換え（今のデータを捨てる）" : "Replace (discard current data)"}
                      </label>
                    </div>
                    <div className="row" style={{ gap: 10, marginTop: 8, flexWrap: "wrap" }}>
                      <label className="btn" style={{ cursor: "pointer" }}>
                        {uiLang === "ja" ? "地点CSVを選ぶ…" : "Choose places CSV…"}
                        <input type="file" accept=".csv,text/csv" style={{ display: "none" }}
                          onChange={(e) => {
                            const f = e.target.files?.[0];
                            e.target.value = "";
                            if (f) void importPoisCsvFile(f);
                          }} />
                      </label>
                      <label className="btn" style={{ cursor: "pointer" }}>
                        {uiLang === "ja" ? "カテゴリCSVを選ぶ…" : "Choose categories CSV…"}
                        <input type="file" accept=".csv,text/csv" style={{ display: "none" }}
                          onChange={(e) => {
                            const f = e.target.files?.[0];
                            e.target.value = "";
                            if (f) void importCatsCsvFile(f);
                          }} />
                      </label>
                    </div>
                  </div>
                  ...
                  {(cfg.indoor.floors ?? []).length === 0 ? (
                    <div style={{ padding: "10px 12px", background: "var(--card2)", borderRadius: 12, border: "1px solid var(--line)" }}>
                      ...
                      <div className="row" style={{ marginTop: 8, alignItems: "center", gap: 8 }}>
                        <span className="hint">{uiLang === "ja" ? "または" : "or"}</span>
                        <button className="btn soft" style={{ fontSize: 13 }} onClick={() => setFloorEditorTarget("")}>
                          {uiLang === "ja" ? "🖊️ フロアを描いて作る" : "🖊️ Draw a floor plan"}
                        </button>
                      </div>
                      {builderAssets.floorFile ? (
                        ...
                        return (
                          <div key={floor.id} style={{
                            ...
                            <div style={{ marginTop: 8 }}>
                              ...
                              <button className="btn soft" style={{ cursor: "pointer", fontSize: 12, marginLeft: 6 }} onClick={() => setFloorEditorTarget(floor.id)}>
                                {uiLang === "ja" ? "🖊️ 描いて作る" : "🖊️ Draw"}
                              </button>
                              {floorFileForThis ? (
      ...
      {floorEditorTarget !== null ? (
        <FloorPlanEditor
          uiLang={uiLang}
          onCancel={() => setFloorEditorTarget(null)}
          onComplete={(file, w, h) => {
            if (floorEditorTarget === "") {
              // Single-floor target
              setBuilderAsset("floor", "floor", file);
              setBuilderConfig({ ...cfg, indoor: { ...cfg.indoor, imageWidthPx: w, imageHeightPx: h } });
            } else {
              // Specific multi-floor entry
              setBuilderAsset("floorMulti", floorEditorTarget, file);
              const floors = [...(cfg.indoor.floors ?? [])];
              const fi = floors.findIndex(f => f.id === floorEditorTarget);
              if (fi >= 0) {
                floors[fi] = { ...floors[fi], imageWidthPx: w, imageHeightPx: h };
                setBuilderConfig({ ...cfg, indoor: { ...cfg.indoor, floors } });
              }
            }
            setFloorEditorTarget(null);
            toast.success(uiLang === "ja" ? "フロア画像を取り込みました" : "Floor image imported");
          }}
        />
      ) : null}

      {/* Research mode (opt-in via #/builder?research=1; invisible otherwise) */}
      <ResearchPanel uiLang={uiLang} step={step} floorEditorOpen={floorEditorTarget !== null} />
    </main>
  );
}
