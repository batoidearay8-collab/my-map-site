...
export const SYMBOLS: { key: string; ja: string; en: string; glyph: string }[] = [
...
export const ROOM_COLORS: { key: string; ja: string; en: string; fill: string; stroke: string }[] = [
...
const fillOf = (key: string) => ROOM_COLORS.find((c) => c.key === key)?.fill ?? DEFAULT_FILL;

/**
 * One-click room templates ("stamp" a typical school room).
 * Sizes are in canvas px, designed against the 2000×1400 base size.
 * `symbol` (optional) also places a matching symbol marker; `symbolDy` shifts
 * it vertically as a fraction of the room height (e.g. -0.25 = above center)
 * so it doesn't collide with the room label.
 */
type RoomTemplate = {
  key: string; ja: string; en: string;
  w: number; h: number;
  colorKey: string;
...
export const ROOM_TEMPLATES: RoomTemplate[] = [
  { key: "class",    ja: "教室",     en: "Classroom", w: 500,  h: 350, colorKey: "class",    labelJa: "教室",     labelEn: "Classroom" },
  { key: "special",  ja: "特別教室", en: "Special",   w: 600,  h: 400, colorKey: "special",  labelJa: "特別教室", labelEn: "Special room" },
  { key: "stairs",   ja: "階段",     en: "Stairs",    w: 250,  h: 350, colorKey: "facility", labelJa: "",         labelEn: "",         symbol: "stairs" },
  { key: "toilet",   ja: "トイレ",   en: "Restroom",  w: 250,  h: 300, colorKey: "water",    labelJa: "",         labelEn: "",         symbol: "toilet" },
  { key: "entrance", ja: "昇降口",   en: "Entrance",  w: 450,  h: 250, colorKey: "default",  labelJa: "昇降口",   labelEn: "Entrance", symbol: "entrance", symbolDy: -0.28 },
  { key: "gym",      ja: "体育館",   en: "Gym",       w: 900,  h: 600, colorKey: "special",  labelJa: "体育館",   labelEn: "Gym" },
  { key: "corridor", ja: "廊下",     en: "Corridor",  w: 1200, h: 150, colorKey: "facility", labelJa: "廊下",     labelEn: "Corridor" },
];

/** Output canvas size presets. Larger = more detail, heavier PNG. */
export const SIZE_PRESETS: { key: string; ja: string; en: string; w: number; h: number }[] = [
  { key: "std",   ja: "標準", en: "Standard", w: 2000, h: 1400 },
  { key: "large", ja: "大",   en: "Large",    w: 3000, h: 2100 },
  { key: "xl",    ja: "特大", en: "XL",       w: 4000, h: 2800 },
  { key: "tall",  ja: "縦長", en: "Portrait", w: 1400, h: 2000 },
];
...
export function FloorPlanEditor({
  ...
  // Undo history: each snapshot holds BOTH rooms and symbols so a template
  // stamp (room + symbol) or a symbol edit is reverted in one step.
  const [history, setHistory] = useState<{ rects: Rect[]; markers: Marker[] }[]>([]);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [floorName, setFloorName] = useState("");
  const [snap, setSnap] = useState(true);
  // Output canvas size (canvas px). Changing it keeps existing content as-is.
  const [canvasW, setCanvasW] = useState(outWidth);
  const [canvasH, setCanvasH] = useState(outHeight);
  ...
  const VIEW_W = 720;
  const scale = VIEW_W / canvasW;
  const VIEW_H = Math.round(canvasH * scale);
  ...
  const clampX = (v: number) => Math.max(0, Math.min(canvasW, v));
  const clampY = (v: number) => Math.max(0, Math.min(canvasH, v));
  ...
  /** Snapshot rooms AND symbols onto the undo stack (cap at 50). Call BEFORE a change. */
  const pushHistory = () => {
    setHistory((h) => {
      const next = [...h, { rects: rects.map((r) => ({ ...r })), markers: markers.map((m) => ({ ...m })) }];
  ...
  /**
   * Stamp a template room onto the canvas. Placement is staggered from the
   * top-left so consecutive stamps don't fully overlap; the user then drags
   * it into place. If the template has a symbol, it's placed in the same
   * history step so one Undo removes both.
   */
  const addTemplate = (t: RoomTemplate) => {
    pushHistory();
    metricsCount("template_room");
    const n = rects.length;
    const rawX = 100 + (n % 5) * 120;
    const rawY = 100 + (n % 5) * 90;
    const x = Math.round(Math.min(rawX, Math.max(0, canvasW - t.w)) / GRID) * GRID;
    const y = Math.round(Math.min(rawY, Math.max(0, canvasH - t.h)) / GRID) * GRID;
    const id = "r" + Date.now();
    const label = t.labelJa ? T(uiLang, t.labelJa, t.labelEn) : "";
    setRects((rs) => [...rs, { id, x, y, w: t.w, h: t.h, rotation: 0, label, color: fillOf(t.colorKey) }]);
    if (t.symbol) {
      const mx = x + t.w / 2;
      const my = y + t.h / 2 + (t.symbolDy ?? 0) * t.h;
      setMarkers((ms) => [...ms, { id: "m" + Date.now(), x: Math.round(mx), y: Math.round(my), kind: t.symbol! }]);
    }
    setSelectedId(id);
    setPlaceKind(null);
  };
  ...
  const exportPng = () => {
    ...
    ctx.lineWidth = 1;
    const grid = 100;
    for (let gx = 0; gx <= canvasW; gx += grid) { ctx.beginPath(); ctx.moveTo(gx, 0); ctx.lineTo(gx, canvasH); ctx.stroke(); }
    for (let gy = 0; gy <= canvasH; gy += grid) { ctx.beginPath(); ctx.moveTo(0, gy); ctx.lineTo(canvasW, gy); ctx.stroke(); }
    ...
    canvas.toBlob((blob) => {
      ...
      onComplete(file, canvasW, canvasH);
    }, "image/png");
  };
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 auto" }}>
            ...
            {/* Output image size presets */}
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 13, marginBottom: 4 }}>{T(uiLang, "画像サイズ", "Image size")}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {SIZE_PRESETS.map((s) => {
                  const active = canvasW === s.w && canvasH === s.h;
                  return (
                    <button key={s.key} type="button"
                      onClick={() => { setCanvasW(s.w); setCanvasH(s.h); }}
                      style={{
                        padding: "4px 8px", borderRadius: 8, cursor: "pointer", fontSize: 12,
                        background: active ? "color-mix(in srgb, var(--accent, #2e6db4) 18%, #fff)" : "var(--card, #fff)",
                        border: active ? "2px solid #e0533d" : "1px solid var(--line, #ccc)",
                      }}>
                      {T(uiLang, s.ja, s.en)}<br />
                      <span style={{ color: "var(--muted, #777)" }}>{s.w}×{s.h}</span>
                    </button>
                  );
                })}
              </div>
              {(rects.length > 0 || markers.length > 0) ? (
                <div className="hint" style={{ marginTop: 4 }}>
                  {T(uiLang, "サイズを変えても描いた部屋はそのまま残ります（小さいサイズにすると外にはみ出すことがあります）。",
                    "Changing size keeps your rooms as-is (shrinking may leave some outside the canvas).")}
                </div>
              ) : null}
            </div>

            {/* One-click room templates */}
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 13, marginBottom: 4 }}>{T(uiLang, "テンプレから部屋を追加", "Add a room from template")}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {ROOM_TEMPLATES.map((t) => (
                  <button key={t.key} type="button" title={`${t.w}×${t.h}px`}
                    ...
                    <span style={{
                      ...
                      background: fillOf(t.colorKey), border: `1.5px solid ${strokeFor(fillOf(t.colorKey))}`,
                    }} />
                    {T(uiLang, t.ja, t.en)}
                  </button>
                ))}
              ...
              <div className="hint" style={{ marginTop: 4 }}>
                {T(uiLang, "押すと左上あたりに部屋が置かれます。ドラッグで好きな場所へ移動してください。",
                  "Stamps a room near the top-left; drag it into place.")}
              </div>
            </div>
              ...
              {!traceUrl ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button className="btn soft" onClick={() => setTraceVisible((v) => !v)}>
                      {traceVisible ? T(uiLang, "下絵を隠す", "Hide photo") : T(uiLang, "下絵を表示", "Show photo")}
                    </button>
            ...
            >
              ...
              {/* grid */}
              {Array.from({ length: Math.floor(canvasW / 100) + 1 }).map((_, i) => (
                <line key={"v" + i} x1={i * 100 * scale} y1={0} x2={i * 100 * scale} y2={VIEW_H} stroke="#eef1f5" />
              ))}
              {Array.from({ length: Math.floor(canvasH / 100) + 1 }).map((_, i) => (
            ...
            <div className="hint" style={{ marginTop: 6 }}>
              {T(uiLang, `出力サイズ: ${canvasW}×${canvasH}px ・ 部屋数: ${rects.length} ・ 記号: ${markers.length}`,
                `Output: ${canvasW}×${canvasH}px · Rooms: ${rects.length} · Symbols: ${markers.length}`)}
            </div>
          </div>
          ...
          <div style={{ flex: "0 0 250px", minWidth: 230 }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <button className="btn soft" onClick={() => { if (rects.length || markers.length) pushHistory(); setRects([]); setMarkers([]); setPlaceKind(null); setSelectedId(null); }}>
