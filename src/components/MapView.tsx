 ...
 *  Re-fits whenever the active floor (floorKey) changes, and lowers the map's
 *  minZoom so the whole image can always be zoomed out to fit the viewport. */
function FitIndoorImage({ bounds, floorKey }: { bounds: L.LatLngBounds; floorKey: string }) {
  const map = useMap();
  React.useEffect(() => {
    // Zoom level at which the whole image fits the current viewport.
    const fitZoom = map.getBoundsZoom(bounds, false, L.point(20, 20));
    // Allow zooming out a bit beyond the fit level so nothing is ever clipped.
    map.setMinZoom(Math.floor(fitZoom) - 1);
  ...
  return (
    <div className="mapWrap">
      <MapContainer
        ...
        minZoom={config.mode === "indoor" ? (config.indoor.minZoom ?? -6) : undefined}
      ...
      >
        ...
        {config.mode === "indoor"
          ? <FitIndoorImage bounds={indoorBounds} floorKey={activeFloorId} />
