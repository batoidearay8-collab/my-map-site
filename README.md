...
### ビルダーの作業時間計測モード（v10.22.0 追加・既定で無効）

地図を「作る側」の作業時間を計測する研究モードです。ビルダーのURLに \`?research=1\` を付けたとき（例: \`#/builder?research=1\`）だけ同意ダイアログが表示され、同意した場合のみ計測が始まります。

- **既定で完全に無効**。URLパラメータを知らない一般利用者には一切表示されません。
- 記録するのは、ステップごとのアクティブ作業時間（60秒以上入力が無い時間は休憩として除外）と、操作回数（元に戻す・エラー表示・テンプレート使用など）のみ。氏名等の個人情報は扱わず、参加者は匿名ID（例: P01）で区別します。
- **データはこのブラウザの localStorage にのみ保存され、どこにも送信されません**（本モジュールに通信コードは存在しません。ソースは \`src/lib/builderMetrics.ts\`）。データが外に出るのは、参加者自身が「CSV/JSON書き出し」を押してファイルを研究者に渡したときだけです。
- 参加者はいつでも計測を終了でき、終了時に記録は全て削除されます。
